...
6. **Boekingsdatum Inkoopbonnen (Huidige datum / Betaaldatum):** Inkoopbonnen (zoals huur oefenruimte) moeten ALTIJD worden vastgelegd op de daadwerkelijke betaaldatum (de huidige datum / `new Date()`) in het tabblad van het actuele kwartaal (`Q[huidig kwartaal] Inkoop`). Gebruik hiervoor NOOIT de geanalyseerde exportmaand. Zorg er tevens voor dat bonnen en facturen altijd in de eerstvolgende vrije rij VÓÓR de Totalen-rij worden weggeschreven.
